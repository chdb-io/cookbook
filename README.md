...
- [A data analyst agent with chDB — on Azure Container Apps](azure-container-apps/README.md) — the same analyst as a scale-to-zero serverless container: dataset baked into the image by a server-side ACR build, idle = free, with the cold-start economics measured honestly. *(App and image byte-identical to the Cloud Run recipe.)*
- [A data analyst agent with chDB — on AWS Lambda](aws-lambda/README.md) — the same analyst as a classic Lambda container function: per-request billing, a public Function URL, and a measured 9.9s cold start with 1M rows baked in. *(App byte-identical to the Cloud Run / Container Apps recipes; the Dockerfile adds one Web Adapter line.)*
